from​data.install​import​*


git_pull()
os.chdir('/data/data/com.termux/files/usr/etc/')
usr​=​os.path.exists​('.usuario')
install()


 try:
 ​  ​while​True:
 ​      ​from​time​import​sleep
 ​      ​from​data.menu​import​*
 ​      ​from​data.ferramentas​import​clear
 ​      ​clear()
 ​      ​if​not​usr:
 ​          ​print(f'{cy}~Lursy: {az}OPA...')
 ​          ​sleep(1)
 ​           print('    ESSE E MEU MENU DE PERSONALIZACAO')
 ​          ​sleep(3.5)
 ​           clear()
 ​          ​print(f'{cy}~Lursy:​{az}BY LORINHO404')
 ​           sleep(1)
 ​          ​print('    TEAM 404 DOMINA!!')
 ​          ​sleep(2)
 ​          ​print('    BEM VINDO AO MENU :D')
 ​          ​sleep(1.4)
 ​        ​  print(personf.rstrip(),​'2.0\n\n')
 ​      ​user​=​(open('.usuario',​'r')).readline​()
 ​      ​print(f'''{rx}​━━━━━━━━━━━━━━━━━
 ​  ​{br}[ 1 ] {cy}-{ve}​letra
 ​  ​{br}[ 2 ] {cy}-{ve}​banner
 ​  ​{br}[ 3 ]​{cy}-{ve}​senha
 ​  ​{br}[ 4 ] {cy}-{ve}​{user}
 ​  ​{br}[ 5 ] {cy}-{ve} Criador
 ​  ​{br}[ 6 ]​{cy}-{ve}​Sair
 ​{rx}​━━━━━━━━━━━━━━━━━
​''')
        ​menu​=​str(input(f'{ve}escolha:​{br}))
 ​      ​if​not​usr:
     ​      ​print(f'{vd}~Lursy:​{vd}otima escolha!')
 ​          ​sleep(1)
 ​          ​usr​=​True
        ​if​menu​==​'1'​:
            ​from​data.letra​import​letra
 ​          ​letra()
        ​elif​menu​==​'2':
 ​          ​from​data.banner​import​banner
 ​          ​banner()
 ​      ​elif​menu​==​'3':
            ​from​data.senha​import​senha
 ​          ​senha()
 ​      ​elif​menu​==​'4':
 ​          ​from​data.nome​import​nome_letra
            ​nome_letra()
 ​          ​from​data.ferramentas​import​*
 ​      ​elif​menu​==​'5':
 ​          ​os.system(f'termux-open-url https://youtube.com/channel/UCHle5l7vdC66Nj5QObkc_lg')
 ​          ​clear()
 ​      ​elif​menu​==​'6':
 ​          ​print(f'\n{vd}Saindo...')
 ​          ​sleep(1)
 ​          ​clear()
 ​          ​break
 ​      ​else:
 ​          ​print(f'{ve}Comando não reconhecido')
            ​sleep(1)
except​KeyboardInterrupt:
 ​  ​print(f'\n{vd}volte sempre...')
 ​  ​sleep​(1)
    ​clear​()